...
async function startServer() {
  ...
  app.post("/api/landlords/register", (req, res) => {
    try {
      ...
      const newLandlord = {
        ...
        idNumber: idNumber || `ID-${Math.floor(1e7 + Math.random() * 9e7)}`,
        subscriptionStatus: "Active",
  ...
  app.post("/api/payments/stk-push", (req, res) => {
    ...
    const receiptCode = `SAB${Math.floor(1e7 + Math.random() * 9e7)}`;
    ...
    const receiptEmailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #10b981; border-radius: 12px; background: #fff;">
        <div style="background-color: #065f46; color: white; padding: 16px; border-radius: 8px 8px 0 0; text-align: center;">
          <h2 style="margin: 0; font-size: 20px;">\u{1F4F2} M-PESA PAYMENT CONFIRMED</h2>
        ...
        <div style="padding: 20px 0;">
          ...
          <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 16px; border-radius: 8px; margin: 16px 0;">
            ...
            <p style="margin: 4px 0; color: #166534; font-size: 13px;"><strong>Date & Time:</strong> ${(/* @__PURE__ */ new Date()).toLocaleString("en-KE")}</p>
    ...
    emailLogs.unshift({
      ...
      subject: `\u{1F4F2} M-Pesa Receipt ${receiptCode}: KSh ${payAmt.toLocaleString()} for Invoice #${inv.invoiceNumber}`,
  ...
  app.post("/api/tenants/register", async (req, res) => {
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
          ...
          <div style="padding: 20px 0;">
            ...
            <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 16px; margin: 16px 0; border-radius: 8px;">
              <h3 style="margin: 0 0 10px 0; color: #166534; font-size: 15px;">\u{1F4C4} Initial Documents Dispatched</h3>
      ...
      const welcomeEmail = {
        ...
        subject: `\u{1F389} Registration Confirmed: Unit ${selectedUnit.unitNumber} - ${selectedUnit.propertyName}`,
  ...
  app.post("/api/invoices/generate", (req, res) => {
    ...
    emailLogs.push({
      ...
      subject: `\u{1F4C4} Monthly Rent Invoice #${inv.invoiceNumber} (${inv.periodMonth})`,
  ...
  app.post("/api/quotes/generate", (req, res) => {
    ...
    emailLogs.push({
      ...
      subject: `\u{1F3F7}\uFE0F Rental Quotation #${qte.quoteNumber} - Unit ${qte.unitNumber}`,
...
4. suggestedUrgency: "Emergency" | "High" | "Medium" | "Low"`;
  ...
  app.patch("/api/maintenance/:id", (req, res) => {
    ...
    if (req.body.status === "Completed") reqObj.resolvedAt = (/* @__PURE__ */ new Date()).toISOString();
    res.json(reqObj);
  });
  app.get("/api/emails", (req, res) => {
    const { recipientEmail } = req.query;
    if (recipientEmail) {
      const filtered = emailLogs.filter((e) => e.recipientEmail.toLowerCase() === recipientEmail.toLowerCase());
